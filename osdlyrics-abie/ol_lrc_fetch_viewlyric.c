...
#define SEARCH_URL "http://www.viewlyrics.com:1212/searchlyrics.htm"
#define SEARCH_XML "<?xml version='1.0' encoding='utf-8' standalone='yes' ?><searchV1 filetype=\"lyrics\" artist=\"%s\" title=\"%s\" client=\"MiniLyricsForAndroid\" />"
  ...
  if (strcmp (element_name, "fileinfo") == 0)
  {
         ...
         attr++, value++)
    {
      if (strcmp (*attr, "link") == 0){
        url = g_malloc(25 + strlen(*value));
        sprintf(url, "http://viewlyrics.com/%s", *value);
      }
...
static OlLrcCandidate *_search(const OlMusicInfo *info, int *size, const char* charset)
{
  ...
  {
    ol_errorf ("Search lyrics failed\n");
          ...
          const char *charset)
{
  ...
  else
  {
    ...
    char *url = ol_lrc_candidate_get_url (candidate);
    ol_debugf("Downloading %s\n", url);
    fetch_into_file (url, NULL, fp);
    fclose(fp);
    ret = 0;
  }
  return ret;
