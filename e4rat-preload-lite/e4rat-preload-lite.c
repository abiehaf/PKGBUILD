...
static void load_inodes(const int a, const int b){
    struct stat s;
    int i;
    for(i = a; i < ((b < listlen) ? b : listlen); i++){
...
int main(int argc, char **argv){
    ...
    if (load_list()) {
      ...
      for(i = early_load; i < listlen; i += BLOCK){
      ...
      #if VERBOSE > 0
          printf("Log not found, continue standard boot.\n", listlen);
      #endif
      execv(INIT, argv);
      die(strerror(errno));
    }
